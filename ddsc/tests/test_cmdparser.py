...
class TestCommandParser(TestCase):
    ...
    def test_register_upload_command(self, mock_paths_must_exists):
        ...
        self.assertEqual(False, self.parsed_args.compare_checksum_size_limit.always_hash)
        self.assertEqual(10485760, self.parsed_args.compare_checksum_size_limit.file_size_limit)

    @patch("ddsc.cmdparser._paths_must_exists")
    def test_register_upload_command__20MB_limit(self, mock_paths_must_exists):
        command_parser = CommandParser(version_str='1.0')
        command_parser.register_upload_command(self.set_parsed_args)
        self.assertEqual(['upload'], list(command_parser.subparsers.choices.keys()))
        command_parser.run_command(['upload', '-p', 'myproj', 'myfile.txt', '--compare-checksum-size-limit', '20MB'])
        self.assertEqual('myproj', self.parsed_args.project_name)
        self.assertEqual(None, self.parsed_args.project_id)
        self.assertEqual([mock_paths_must_exists.return_value], self.parsed_args.folders)
        self.assertEqual(False, self.parsed_args.compare_checksum_size_limit.always_hash)
        self.assertEqual(20971520, self.parsed_args.compare_checksum_size_limit.file_size_limit)

    @patch("ddsc.cmdparser._paths_must_exists")
    def test_register_upload_command__1000_limit(self, mock_paths_must_exists):
        command_parser = CommandParser(version_str='1.0')
        command_parser.register_upload_command(self.set_parsed_args)
        self.assertEqual(['upload'], list(command_parser.subparsers.choices.keys()))
        command_parser.run_command(['upload', '-p', 'myproj', 'myfile.txt', '--compare-checksum-size-limit', '1000'])
        self.assertEqual('myproj', self.parsed_args.project_name)
        self.assertEqual(None, self.parsed_args.project_id)
        self.assertEqual([mock_paths_must_exists.return_value], self.parsed_args.folders)
        self.assertEqual(False, self.parsed_args.compare_checksum_size_limit.always_hash)
        self.assertEqual(1000, self.parsed_args.compare_checksum_size_limit.file_size_limit)

    @patch("ddsc.cmdparser._paths_must_exists")
    def test_register_upload_command__always_checksum(self, mock_paths_must_exists):
        command_parser = CommandParser(version_str='1.0')
        command_parser.register_upload_command(self.set_parsed_args)
        self.assertEqual(['upload'], list(command_parser.subparsers.choices.keys()))
        command_parser.run_command(['upload', '-p', 'myproj', 'myfile.txt', '--compare-checksum-size-limit', 'None'])
        self.assertEqual('myproj', self.parsed_args.project_name)
        self.assertEqual(None, self.parsed_args.project_id)
        self.assertEqual([mock_paths_must_exists.return_value], self.parsed_args.folders)
        self.assertEqual(True, self.parsed_args.compare_checksum_size_limit.always_hash)
        self.assertEqual(0, self.parsed_args.compare_checksum_size_limit.file_size_limit)
